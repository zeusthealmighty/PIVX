...
// Possible states for zMRI send
enum ZerocoinSpendStatus {
    ZMRI_SPEND_OKAY = 0,                            // No error
    ZMRI_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMRI_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMRI_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMRI_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMRI_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMRI_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMRI_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMRI_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMRI_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMRI_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMRI_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMRI_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMRI_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMRI_SPENT_USED_ZMRI = 14,                      // Coin has already been spend
    ZMRI_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    /** zMRI reset */
    boost::signals2::signal<void()> NotifyzMRIReset;
};
