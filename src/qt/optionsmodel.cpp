...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMiraiAmount"))
        settings.setValue("nAnonymizeMiraiAmount", 1000);

    nAnonymizeMiraiAmount = settings.value("nAnonymizeMiraiAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMiraiAmount"))
        SoftSetArg("-anonymizemiraiamount", settings.value("nAnonymizeMiraiAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMiraiAmount:
            return QVariant(nAnonymizeMiraiAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMiraiAmount:
            nAnonymizeMiraiAmount = value.toInt();
            settings.setValue("nAnonymizeMiraiAmount", nAnonymizeMiraiAmount);
            emit anonymizeMiraiAmountChanged(nAnonymizeMiraiAmount);
            break;
        case CoinControlFeatures:
