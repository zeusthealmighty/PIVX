...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the MRI and zMRI money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 4444, 51474));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, mirai, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all MRI specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
...
//    strUsage += "  -anonymizemiraiamount=<n>     " + strprintf(_("Keep N MRI anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 4445, 51475));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //MRI specific: zerocoin and spork DB's
                ...
                // MRI: load previous sessions sporks if we have them.
                ...
                // MRI: recalculate Accumulator Checkpoints that failed to database properly
...
//    }
//
//    nAnonymizeMiraiAmount = GetArg("-anonymizemiraiamount", 0);
//    if (nAnonymizeMiraiAmount > 999999) nAnonymizeMiraiAmount = 999999;
//    if (nAnonymizeMiraiAmount < 2) nAnonymizeMiraiAmount = 2;
